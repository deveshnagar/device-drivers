...
static int thread_fn(void *id)
{
	...
	while (!kthread_should_stop()) 
	{
		mutex_lock(&my_mutex);
		counter++;
		printk(KERN_INFO "mutex: Job %d started\n", counter);
		get_random_bytes(&i, sizeof(i));
		ssleep(i % 5);
		printk(KERN_INFO "mutex: Job %d finished\n", counter);
	...
	printk(KERN_INFO "mutex: Thread Stopping\n");
	do_exit(0);
}
...
static int __init init_mutex(void)
{
	...
	{
		printk(KERN_INFO "mutex: Creating Thread %d\n", i);
		thread_id[i] = i;
...
static void __exit cleanup_mutex(void)
{
	...
	printk(KERN_INFO "mutex: Cleaning Up\n");
	for (i = 0; i < TOTAL_THREADS; i++)
	{
		if (thread_st[i] != NULL)
		{
			kthread_stop(thread_st[i]);
			printk(KERN_INFO "mutex: Thread %d stopped", i);
		}
	}
}
...
MODULE_DESCRIPTION("Concurreny Mutex Example");
