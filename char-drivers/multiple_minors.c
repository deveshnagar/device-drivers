...
{
	printk(KERN_INFO "final_simple: myOpen() from = %d:%d\n", imajor(i), iminor(i));
	return 0;
}
static int myClose(struct inode *i, struct file *f)
{
	printk(KERN_INFO "final_simple: myClose() +");
	printk(KERN_INFO "final_simple: myClose() from = %d:%d\n", imajor(i), iminor(i));
	return 0;
}
...
{
	unsigned int ret = 14;
	printk(KERN_INFO "final_simple: myReade() from = %d:%d\n", imajor(f->f_inode), iminor(f->f_inode));
	switch(iminor(f->f_inode))
	{
		case FIRST_MINOR:
			sendData = "Hello First  \n";	//14 bytes size
			break;
		case 2:
			sendData = "Hello Second \n";	//14 bytes size
			break;
		case 3:
			sendData = "Hello Third  \n";	//14 bytes size
			break;
		case TOTAL_MINOR:
			sendData = "Hello Fourth \n";	//14 bytes size
			break;
	}
	//sendData = "Hello World\n"; //12 bytes size
	if(*off == 0)
	{
		ret = copy_to_user(buf, sendData, 14);
		*off += 14;
	}	
	if(ret == 0)
	{
...
{
	ssize_t ret;
	printk(KERN_INFO "final_simple: myWrite() from = %d:%d\n", imajor(f->f_inode), iminor(f->f_inode));
	if(len <= 0)
	{
...
static int __init myInit(void)
{
	...
	device_ret = device_create(final_simple_class, NULL, MKDEV(MAJOR(dev_no), FIRST_MINOR), NULL, "final_simple%d", FIRST_MINOR);
	for(i = (FIRST_MINOR+1); i <= TOTAL_MINOR; i++)
	{
		device_create(final_simple_class, device_ret, MKDEV(MAJOR(dev_no), i), NULL, "final_simple%d", (int)i);
...
{
	for(i = TOTAL_MINOR; i >= FIRST_MINOR; i--)
	{
		device_destroy(final_simple_class, MKDEV(MAJOR(dev_no), i) );
	}
	class_destroy(final_simple_class);
	cdev_del(&char_dev);
	unregister_chrdev_region(dev_no, TOTAL_MINOR);
